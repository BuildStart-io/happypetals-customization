...
async function processMessage(
  ...
  try {
    aiResponse = await fetch(`${supabaseUrl}/functions/v1/ai-chat-happypetal-customization`, {
      method: "POST",
      headers: {
...
async function sendWhatsApp(
  ...
  const res = await fetch(`${supabaseUrl}/functions/v1/send-whatsapp-happypetal-customization`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${supabaseServiceKey}`,
      "Content-Type": "application/json",
